    ...
    for xi in np.arange(begx,endx,zone_step):
        ...
        for yi in np.arange(begy,endy,zone_step):
            if in_poligon(geometry,[xi,yi]):
                dop_map[i][j]=dop_of_point(stas,[xi,yi,25])[0]
            j+=1
        i+=1
...
'''
geometry = np.array ([[0,0],[0,zonе_height],[zone_widtch*2,zonе_height*2],[zone_widtch,0]])
...
#шаг по зоне
zone_stepx=endx/5#float(input("Введите точность"))
zone_stepy=endy/5
    ...
    for yi1 in np.arange(begy,endy+0.00000001,zone_stepy):
        ...
        for xi2 in np.arange(begx,endx+0.00000001,zone_stepx):
            ...
            for yi2 in np.arange(begy,endy+0.00000001,zone_stepy):
                ...
                if(np.abs(stations[0][0]-stations[1][0])<=min_distx) and(np.abs(stations[0][1]-stations[1][1])<=min_disty): continue
                ...
                for xi3 in np.arange(begx,endx+0.00000001,zone_stepx):
                    ...
                    for yi3 in np.arange(begy,endy+0.00000001,zone_stepy):
                        stations[2][1]=yi3
                        if(np.abs(stations[1][0]-stations[2][0])<=min_distx) and(np.abs(stations[1][1]-stations[2][1])<=min_disty): continue
                        if(np.abs(stations[0][0]-stations[2][0])<=min_distx) and(np.abs(stations[0][1]-stations[2][1])<=min_disty): continue
                        ...
                        for xi4 in np.arange(begx,endx+0.00000001,zone_stepx):
                            stations[3][0]=xi4
                            j4=0
                            for yi4 in np.arange(begy,endy+0.00000001,zone_stepy):
                                stations[3][1]=yi4
                                if(np.abs(stations[0][0]-stations[3][0])<=min_distx) and(np.abs(stations[0][1]-stations[3][1])<=min_disty): continue
                                if(np.abs(stations[1][0]-stations[3][0])<=min_distx) and(np.abs(stations[1][1]-stations[3][1])<=min_disty): continue
                                if(np.abs(stations[2][0]-stations[3][0])<=min_distx) and(np.abs(stations[2][1]-stations[3][1])<=min_disty): continue
